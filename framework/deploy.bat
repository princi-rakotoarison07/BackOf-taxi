@echo off
set FRAMEWORK_DIR=c:\Users\Princi\Documents\framework
set SERVLET_JAR=%FRAMEWORK_DIR%\jakarta.servlet-api_5.0.0.jar
set BUILD_DIR=build
set DEST_DIR="d:\mahasoa\ITU\S5\INFO302-TechnoAccesReseaux\inDrive\BackOf-taxi\modules"
...
javac -d "%BUILD_DIR%\classes" "%FRAMEWORK_DIR%\framework\annotation\*.java"

REM 2. Compilation des utilitaires
echo [2/4] Compilation des utilitaires...
javac -classpath "%SERVLET_JAR%;%BUILD_DIR%\classes" -d "%BUILD_DIR%\classes" "%FRAMEWORK_DIR%\framework\utilitaire\*.java"

REM 3. Compilation des servlets
echo [3/4] Compilation des servlets...
javac -classpath "%SERVLET_JAR%;%BUILD_DIR%\classes" -d "%BUILD_DIR%\classes" "%FRAMEWORK_DIR%\framework\servlet\*.java"
